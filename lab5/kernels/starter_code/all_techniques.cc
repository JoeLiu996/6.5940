    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                int8x16_t w0_lower_half = vreinterpretq_s8_u8(vandq_u8(w0, mask_low4bit));
                int8x16_t w0_upper_half = vreinterpretq_s8_u8(vshrq_n_u8(w0, 4));
                int8x16_t w1_lower_half = vreinterpretq_s8_u8(vandq_u8(w1, mask_low4bit));
                int8x16_t w1_upper_half = vreinterpretq_s8_u8(vshrq_n_u8(w1, 4));
                int8x16_t w2_lower_half = vreinterpretq_s8_u8(vandq_u8(w2, mask_low4bit));
                int8x16_t w2_upper_half = vreinterpretq_s8_u8(vshrq_n_u8(w2, 4));
                int8x16_t w3_lower_half = vreinterpretq_s8_u8(vandq_u8(w3, mask_low4bit));
                int8x16_t w3_upper_half = vreinterpretq_s8_u8(vshrq_n_u8(w3, 4));
                ...
                w0_lower_half = vsubq_s8(w0_lower_half, offsets);
                w0_upper_half = vsubq_s8(w0_upper_half, offsets);
                w1_lower_half = vsubq_s8(w1_lower_half, offsets);
                w1_upper_half = vsubq_s8(w1_upper_half, offsets);
                w2_lower_half = vsubq_s8(w2_lower_half, offsets);
                w2_upper_half = vsubq_s8(w2_upper_half, offsets);
                w3_lower_half = vsubq_s8(w3_lower_half, offsets);
                w3_upper_half = vsubq_s8(w3_upper_half, offsets);
                ...
                int32x4_t int_sum0 = vdupq_n_s32(0), int_sum1 = vdupq_n_s32(0), int_sum2 = vdupq_n_s32(0), int_sum3 = vdupq_n_s32(0);
                int_sum0 = vdotq_s32(int_sum0, a0, w0_lower_half);
                int_sum0 = vdotq_s32(int_sum0, a1, w0_upper_half);
                int_sum1 = vdotq_s32(int_sum1, a2, w1_lower_half);
                int_sum1 = vdotq_s32(int_sum1, a3, w1_upper_half);
                int_sum2 = vdotq_s32(int_sum2, a4, w2_lower_half);
                int_sum2 = vdotq_s32(int_sum2, a5, w2_upper_half);
                int_sum3 = vdotq_s32(int_sum3, a6, w3_lower_half);
                int_sum3 = vdotq_s32(int_sum3, a7, w3_upper_half);
                float s_0 = *s_a++ * *s_w++;
                float s_1 = *s_a++ * *s_w++;
                float s_2 = *s_a++ * *s_w++;
                float s_3 = *s_a++ * *s_w++;
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    const int thread_per_col = C->column / num_thread;

    // TODO: Thread creation
    for (int i = 0; i < num_thread; i++) {
        threads_args[i].start_j = i * thread_per_col;
        threads_args[i].end_j = (i + 1) * thread_per_col;
